...
struct RunDefinition {
    connected_brokers: Vec<(Vec<u8>, Vec<Topic>)>,
    connected_users: Vec<Vec<Topic>>,
}
...
struct Run {
    ...
    connected_brokers: Vec<InjectedActor>,

    /// The connected users and their handles
    connected_users: Vec<InjectedActor>,
}
...
impl RunDefinition {
    ...
    async fn inject_users(
        ...
        for (i, topics) in users.iter().enumerate() {
            ...
            let subscribe_message = Message::Subscribe(topics.to_vec());
